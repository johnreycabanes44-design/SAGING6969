...
/**
 * Rounded-rectangle stroke drawn by a signed-distance shader, so one outline is
 * six vertices regardless of corner radius or thickness.
 *
 ...
 * {@code outline_fragment.fsh} — change one and the other must follow.
 */
public class OutlinePipeline {
	...
	public static void init() {
		...
		try {
			pipeline = RenderPipeline.builder()
					.withLocation(Identifier.fromNamespaceAndPath("zenya", "outline"))
					.withVertexShader(Identifier.fromNamespaceAndPath("zenya", "outline_vertex"))
					.withFragmentShader(Identifier.fromNamespaceAndPath("zenya", "outline_fragment"))
			...
			// whole client down, it just leaves outlines undrawn.
			System.err.println("[Outline2D] Failed to init: " + failure.getMessage());
			failure.printStackTrace();
		}
	}

	/**
	 * Uploads the uniform block and draws the stroke as two triangles. Corner radii
	 * run clockwise from the top left; {@code colors} is the 3x3 gradient grid the
	...
	public static void draw(Matrix4f projection, float x, float y, float width, float height,
			...
			float thickness, float z, int... colors) {
		if (pipeline == null) {
			init();
		}
		...
		// uThickness, uZ, then two floats of padding.
		uniforms.putFloat(thickness).putFloat(z).putFloat(0.0f).putFloat(0.0f);
		...
		try (RenderPass pass = encoder.createRenderPass(() -> "Outline2D", target.getColorTextureView(),
